...
// The inverse of buildItemAttributes — splits an existing item's flat attributes map back into
// "matches one of the product type's declared attributes" vs. "one-off custom field", so editing
// an already-placed item's attributes can reuse the same two-part editor as adding one.
export function splitItemAttributes(
  attributes: AttributeValues,
  productType: ProductType | undefined
): { attributeValues: Record<string, string>; customAttributes: CustomAttribute[] } {
  const definedNames = new Set((productType?.attributeDefinitions ?? []).map((def) => def.name));
  const attributeValues: Record<string, string> = {};
  const customAttributes: CustomAttribute[] = [];
  for (const [key, value] of Object.entries(attributes)) {
    const stringValue = value === null || value === undefined ? "" : String(value);
    if (definedNames.has(key)) attributeValues[key] = stringValue;
    else customAttributes.push({ key, value: stringValue });
  }
  return { attributeValues, customAttributes };
}
...
interface Props {
  ...
  // Editing an already-placed item's attributes reuses this same component, but the product,
  // quantity, delivery date and price are fixed by then — only the attribute/custom-field/
  // factory-notes sections make sense to show.
  hideProductFields?: boolean;
  onChange: (patch: Partial<OrderItemDraft>) => void;
}

// Shared by the new-order form, the "add a product" modal, and the "edit this item's attributes"
// modal on an existing order, so all three go through the same attribute rendering and
// custom-field editor.
