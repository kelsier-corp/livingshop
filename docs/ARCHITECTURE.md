...
- **`/all` routes** (`GET /product-types/all`, `GET /product-categories/all`) return a plain unpaginated array. These back `ProductTypePicker` and `CategorySelector`, which need the *whole* catalog client-side for instant filtering — safe because that catalog is small reference data (see rule above), not something that scales with orders.
